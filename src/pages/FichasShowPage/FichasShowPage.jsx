...
export default function FichaShowPage() {
    ...
    const { ficha, loading, isEditing, form, errors, saving, startEdit, cancelEdit, onChange, save, deleteFicha, setCurrentTerm, deleteFichaTerm } = useFichaShow(fichaId);
                    ...
                    left: [
                        {
                            ...
                            content: isEditing ? (
                                <>
                                    <InputField label="Número de Ficha" name="ficha_number" value={form.ficha_number} onChange={onChange} error={errors.ficha_number} disabled={saving} />
                                    <InputField label="Programa" name="training_program_id" value={form.training_program_id} onChange={onChange} options={programsCatalog.options} disabled={programsCatalog.loading || saving} error={errors.training_program_id} select />
                                    <InputField label="Gestor" name="gestor_id" value={form.gestor_id} onChange={onChange} options={gestorsCatalog.options} disabled={gestorsCatalog.loading || saving} error={errors.gestor_id} select />
                                </>
                            ) : (
                                <>
                    ...
                    right: [
                        {
                            ...
                            content: isEditing ? (
                                <>
                                    <InputField label="Fecha Inicio" name="start_date" type="date" value={form.start_date} onChange={onChange} error={errors.start_date} disabled={saving} />
                                    <InputField label="Fecha Fin" name="end_date" type="date" value={form.end_date} onChange={onChange} error={errors.end_date} disabled={saving} />
                                    <InputField label="Estado" name="status_id" value={form.status_id} onChange={onChange} options={statusCatalog.options} disabled={statusCatalog.loading || saving} error={errors.status_id} select />
                                </>
                            ) : (
                                <>
    ...
    const actions = useMemo(
        ...
        [isEditing, saving, cancelEdit, save, startEdit, deleteFicha],
    );
