...
export default function useFichaShow(id) {
  ...
  /**
   * Establece término actual para edición (placeholder)
   */
  const setCurrentTerm = useCallback((term) => {
    console.log("setCurrentTerm:", term);
    // Implementar según tu lógica de términos
  }, []);

  /**
   * Elimina término específico de la ficha (placeholder)
   */
  const deleteFichaTerm = useCallback(async (termId) => {
    console.log("deleteFichaTerm:", termId);
    // Implementar según tu lógica específica
    return true;
  }, []);

  return {
    ficha, loading, isEditing, form, errors, saving,
    startEdit, cancelEdit, onChange, save,
    deleteFicha, 
    refetch: fetchFicha, 
    setCurrentTerm, 
    deleteFichaTerm
