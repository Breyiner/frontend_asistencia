...
/**
 * Schema de validación para creación de ambientes.
 * 
 ...
 * @type {Array<Object>}
 */
const classroomCreateSchema = [
  { name: "name", type: "text", required: true, maxLength: 80 },
  { name: "description", type: "text", required: false, minLength: 3, maxLength: 255 },
];

/**
 * Hook personalizado para gestionar la creación de ambientes.
 * 
 * Maneja el ciclo completo de creación de un ambiente:
 * - Estado del formulario
 * - Validación de campos
 * - Envío a la API
 * - Reseteo del formulario
 * 
 * Los ambientes son espacios físicos (ej: "Aula 101", "Laboratorio 2A")
 * que se asignan a programas de formación.
 * 
 * Características:
 ...
 * - Retorna ID del ambiente creado
 * 
 * @hook
 * 
 * @returns {Object} Objeto con estado y funciones del formulario
 * @returns {Object} return.form - Valores del formulario
 * @returns {string} return.form.name - Nombre del ambiente
 * @returns {string} return.form.description - Descripción del ambiente (opcional)
 ...
 * @returns {Function} return.validateAndSave - Valida y crea el ambiente
 * @returns {Function} return.resetForm - Resetea el formulario
 * 
 * @example
 * function CreateClassroomPage() {
 *   const { form, errors, loading, onChange, validateAndSave, resetForm } = useClassroomCreate();
 * 
 *   const handleSubmit = async (e) => {
 *     e.preventDefault();
 *     const result = await validateAndSave();
 *     if (result.ok) {
 *       navigate(`/classrooms/${result.createdId}`);
 *     }
 *   };
 * 
 *   return (
 ...
 *         textarea
 *       />
 *       <button type="submit" disabled={loading}>Crear Ambiente</button>
 *     </form>
 *   );
 * }
 */
export default function useClassroomCreate() {
  ...
  /**
   * Valida el formulario y crea el ambiente en el servidor.
   * 
   * Proceso:
   ...
   * const result = await validateAndSave();
   * if (result && result.ok) {
   *   console.log("Ambiente creado con ID:", result.createdId);
   *   resetForm();
   * }
   */
  const validateAndSave = async () => {
    // Validación con schema
    const result = validarCamposReact(form, classroomCreateSchema);
    if (!result.ok) {
    ...
    try {
      ...
      if (!res.ok) {
        await error(res.message || "No se pudo crear el ambiente.");
        return false;
      }

      // Extrae el ID del ambiente creado
      const createdId = res?.data?.id ?? null;

      await success(res.message || "Ambiente creado con éxito.");
   ...
   * Útil después de crear un ambiente o al cancelar.
   */
  const resetForm = () => {
